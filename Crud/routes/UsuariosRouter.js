...
router.patch('/:id', async(req, res)=>{
    ...
    try{
        await client.connect()
        const result = await client.db('Pach_OS').collection('Usuarios').updateOne({_id: new ObjectId(id)}, {$set:{num_documento: body.num_documento, nombre: body.nombre, celular: body.celular, correo: body.correo}})
        res.status(200).json({

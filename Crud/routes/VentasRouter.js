...
router.get('/:id', async(req, res)=>{
    ...
    try{
        await client.connect()
        const result = await client.db('Pach_OS').collection('Ventas').findOnr({_id: new ObjectId(id_Venta) })
...
router.post('/:id', async(req, res)=>{
    ...
    try{
        await client.connect()
        const result = await client.db('Pach_OS').collection('Ventas').updateOne({_id: new ObjectId(id_Venta)}, {$set:{id_Producto: 53}})
        res.status(200).json({
...
router.delete('/:id', async(req, res)=>{
    ...
    try{
        await client.connect()
        const result = await client.db('Pach_OS').collection('Ventas').deleteOne({_id: new ObjectId(id_Venta)})
        res.status(200).json({
